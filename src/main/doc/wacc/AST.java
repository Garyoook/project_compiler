...
public abstract class AST {
  ...
  public static boolean is_int(AST expr1) {
    if (expr1 instanceof IdentNode) {
      if (!symbolTable.getVariable(((IdentNode) expr1).ident).equals(Type.intType())) {
        return false;
      }
    } else
  ...
  public static boolean is_bool(AST exp) {
    if (exp instanceof IdentNode) {
      if (symbolTable.getVariable(((IdentNode) exp).ident) == null) {
        System.out.println("Semantic error: variable not defined: " + ((IdentNode) exp).ident);
        exit(200);
      }
      if (!symbolTable.getVariable(((IdentNode) exp).ident).equals(Type.boolType())) {
        return false;
      }
    } else
  ...
  public static boolean is_Pair(AST exp) {
    if (exp instanceof IdentNode) {
      return symbolTable.getVariable(((IdentNode) exp).ident) instanceof Type.PairType;
    } else {
  ...
  public static boolean is_Char(AST exp) {
    if (exp instanceof IdentNode) {
      if (!symbolTable.getVariable(((IdentNode) exp).ident).equals(Type.charType())) {
        return false;
      }
    } else
  ...
  public static boolean is_String(AST exp) {
    if (exp instanceof IdentNode) {
      return symbolTable.getVariable(((IdentNode) exp).ident).equals(Type.stringType());
    }
